...
if len(override_diff) > 0:
    override_comment = f"""\
There are also some intents associations that would have changed, had the
intent not been overridden in `files/gnome-intentapps/quirks.toml`. Please
...
[default intentapps.list](https://gitlab.gnome.org/GNOME/gnome-session/-/blob/main/data/gnome-intentapps.list)!
...
stamp = f"<!-- intentapps-bot {fingerprint} -->"
comment = f"{stamp}\n\n{comment}"
...
for discussion in mr.discussions.list(iterator=True):
    ...
    if "<!-- intentapps-bot" in note.body:
